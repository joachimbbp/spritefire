...
func InsertInTree(current *Node, color util.Rgb, spriteName string, depth int) *Node {
	if current == nil {
		return &Node{color: color, spriteName: spriteName, left: nil, right: nil}
	}

	axis := depth % 3

	if current.color.Get(axis) < color.Get(axis) {
		current.right = InsertInTree(current.right, color, spriteName, depth+1)
	} else {
		current.left = InsertInTree(current.left, color, spriteName, depth+1)
	}
...
func (tree *KDTree) Insert(color util.Rgb, spriteName string) {
	tree.root = InsertInTree(tree.root, color, spriteName, 0)
}
